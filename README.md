...
ContentBridge 是一个面向创作者的多平台内容发布工具 MVP。用户先用一句话告诉 Agent 想发什么、面向谁、发到哪些平台，系统再生成平台化改写版本，并提供爆品灵感、发布体检、账号矩阵任务、平台预览编辑和 Webhook 真实投递能力。
...
- 一句话进入主流程：用户先和 Agent 对话，系统再生成内容草稿和目标平台。
...
正式发布到公众号、知乎、B站、小红书、微博、抖音通常需要平台开放接口、账号授权、内容审核和密钥配置。本 MVP 不伪造官方发布结果，而是提供一条可真实跑通的投递链路：
...
如果未来获得官方平台 API、OAuth、Cookie Vault 或浏览器自动化权限，可以把 Webhook 接收端替换为带授权的 Publisher 服务，前端 Agent、平台适配器、发布体检和矩阵任务模型不需要重写。
...
- 账号连接：展示各平台登录/创作入口和连接状态，当前为演示授权状态。
- 热点洞察：根据平台任务生成热点方向和改写建议。
- 真实投递：真实发送 HTTP POST 到 Webhook / 自建服务 / 自动化机器人接收端。
...
原创功能主要包括：Agent 对话入口、爆品灵感库、平台适配器架构、6 个内置平台规则、发布体检规则、账号矩阵运营引擎、Webhook 真实投递、页面交互和项目文档。第三方库仅用于前端框架、构建和测试。
