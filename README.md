...
ContentBridge 是一个面向普通创作者的多平台内容发布助手 MVP。用户先用一句话说明想发布什么，系统再分析需求、生成初稿、选择平台、改写不同平台版本，并提供发布体检、平台预览、人工编辑、发布编排、本地发布记录、外部接收端真实投递和自定义平台扩展能力。
...
- Agent-first 主流程：普通用户先描述需求，助手再调用载入示例、选择平台、生成初稿、配置外部接收端和新增平台等功能。
- 平台风格适配：不同平台生成不同标题、正文结构、标签和互动引导。
- 发布体检：按平台规则给出健康分、风险提示和修改建议。
- 高级工作台：逐平台调稿、编辑/预览切换、恢复生成版本、复制 Markdown、导出全部平台内容默认收起，需要时再展开。
...
- 一句话输入：用户可以用自然语言说明主题、受众、平台和投递方式。
- 内容输入：高级模式下可手动编辑标题、正文、标签、封面图、视频链接、目标平台。
