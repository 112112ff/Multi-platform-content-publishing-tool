...
ContentBridge 是一个面向创作者的多平台内容发布工具 MVP，对应比赛题目二“多平台内容发布工具”。用户用一句话告诉 Agent 想发什么，系统会判断最合适的单个平台，生成平台化草稿，并引导用户确认账号后发送到平台创作页或测试接收端。
...
- Agent 主流程：用户自然语言输入，MiniMax Agent 生成单平台发布计划。
- 安全降级：未配置 MiniMax key 时，自动使用本地 Agent 规则，项目仍可演示。
- 单平台账号确认：输入后只弹出当前目标平台账号，不再一次展示六个平台配置。
- 平台化改写：支持公众号、知乎、B站、小红书、微博、抖音。
- 发布体检：根据标题、平台类型、素材情况给出风险提示。
- 浏览器扩展 Publisher Bridge：复用用户自己的浏览器登录态，打开平台创作页并尝试填充草稿。
...
如果未启动代理或未配置 key，页面会自动降级到本地 Agent。MiniMax 接入细节见 [docs/10-minimax-agent-integration.md](./docs/10-minimax-agent-integration.md)。

## 真实发布边界

正式发布到公众号、知乎、B站、小红书、微博、抖音通常需要平台开放接口、账号授权、内容审核和密钥配置。本 MVP 不伪造官方发布成功，而是提供两条可跑通链路：

1. 浏览器扩展路线：把草稿发送给本地扩展，扩展打开平台创作页并尝试填充，最终由用户人工确认发布。
2. 测试接收端路线：把草稿真实 POST 到用户填写的接收 URL，用于验证 payload、状态处理和失败反馈。
